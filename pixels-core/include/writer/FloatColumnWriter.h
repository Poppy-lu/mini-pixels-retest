...
public:
    FloatColumnWriter(std::shared_ptr<TypeDescription> type, std::shared_ptr<PixelsWriterOption> writerOption);
...
private:
    void writeCurPartFloat(std::shared_ptr<ColumnVector> columnVector, float* values, int curPartLength, int curPartOffset);
    ...
    std::vector<float> curPixelVector; // 当前像素的值向量
};
