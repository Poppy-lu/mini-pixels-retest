...
public:
    DoubleColumnWriter(std::shared_ptr<TypeDescription> type, std::shared_ptr<PixelsWriterOption> writerOption);
...
private:
    void writeCurPartDouble(std::shared_ptr<ColumnVector> columnVector, double* values, int curPartLength, int curPartOffset);
    ...
    std::vector<double> curPixelVector; // 当前像素的值向量
};
