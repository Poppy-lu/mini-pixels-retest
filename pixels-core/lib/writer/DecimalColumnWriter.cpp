...
// 写入数据：将 ColumnVector 中的 Decimal 数据写入当前像素
int DecimalColumnWriter::write(std::shared_ptr<ColumnVector> vector, int size) {
    ...
    int64_t* values = columnVector->decimalVector; // 获取 Decimal 数据
...
// 写入当前分区 Decimal 数据
void DecimalColumnWriter::writeCurPartDecimal(std::shared_ptr<ColumnVector> columnVector, int64_t* values, int curPartLength, int curPartOffset) {
...
void DecimalColumnWriter::newPixel() {
    if (runlengthEncoding) {
        std::vector<byte> buffer(curPixelVectorIndex * sizeof(int64_t));
        int resLen;
        encoder->encode(curPixelVector.data(), buffer.data(), curPixelVectorIndex, resLen);
        outputStream->putBytes(buffer.data(), resLen);
    } else {
        std::shared_ptr<ByteBuffer> curVecPartitionBuffer = std::make_shared<ByteBuffer>(curPixelVectorIndex * sizeof(int64_t));
